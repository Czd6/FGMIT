...
class Local_Co_attention(nn.Module):

    def __init__(self, hidden_size, num_heads, attn_drop=0.):
        super().__init__()
    ...
    def forward(self, x, y, y_mask):
        ...
        linear = nn.Linear(y.shape[-1], C).to(device=0)
        y = linear(y)

        all_windows = [x[:, i:i + 1, :, :] for i in range(x.shape[1])]
        co_att = []
        for local in all_windows:
            window = torch.squeeze(local,1)
            local_window_score = self.att(y, y, window, y_mask)
            co_att.append(local_window_score)

        final_att = torch.stack(co_att, dim=1)
...
class MSGBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, mlp_ratio=4.,
        ...
        self.co_Attn = Local_Co_attention(
            hidden_size=dim, num_heads=num_heads, attn_drop=attn_drop
        )
        ...
        self.global_conv = CSAB(dim, compress_ratio=3, squeeze_factor=30)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, y, y_mask):
        ...
        shotcut = x
        x = self.norm1(x)
        x = self.co_Attn(x,y,y_mask)
        x = shotcut + x
        # CSA
        conv_x = self.global_conv(x.permute(0,3,1,2))
...
class MSGTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        elif manip_type == 'co_attention':
            manip_op = Local_Co_attention
        else:
    ...
    def forward_features(self, x, y, y_mask):
        ...
        x = torch.cat((msg_tokens, x), dim=2)  # [128, 16, 50, 96]

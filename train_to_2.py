...
class onerun:
    ...
    def calculate_parameters(self):
        ...
        for param in self.model.parameters():
            ...
            if param.requires_grad:
                Trainable_params = Trainable_params + mulValue
            else:
                NonTrainable_params = NonTrainable_params + mulValue
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(self.dataloaders["train"]):
            ...
            # scheduler.step()
            self.model.zero_grad()
            running_loss = running_loss + loss.item() * input["label"].size(0)
        ...
        self.log.info(
            "train : F1: {:.4f}, Precision: {:.4f}, Recall : {:.4f}, Accuracy: {:.4f}, Loss: {:.4f}.".format(f1, pre,
                                                                                                             recall,
                                                                                                             epoch_acc,
                                                                                                             epoch_loss))
        return {
    ...
    def eval(self, mode, epoch=None):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.dataloaders[mode]):
                ...
                running_corrects = running_loss + (preds == input["label"]).sum()
                y_pred.extend(preds.tolist())
        ...
        # save predict
        if epoch:
            np.savez(self.predict_save_path[:-4] + str(epoch) + '.npy', y_pred=np.array(y_pred),
                     y_true=np.array(y_true), score=np.array(scores_list))
            np.save(self.predict_save_score_path[:-4] + str(epoch) + '.npy', np.array(scores_list))

        if "pth.tar" in self.info["test_on_checkpoint"]:
            print(mode + ": F1: {:.4f}, Precision: {:.4f}, Recall : {:.4f}, Accuracy: {:.4f}, Loss: {:.4f}.".format(f1,
                                                                                                                    pre,
                                                                                                                    recall,
                                                                                                                    epoch_acc,
                                                                                                                    epoch_loss))
            self.log.info(
                mode + ": F1: {:.4f}, Precision: {:.4f}, Recall : {:.4f}, Accuracy: {:.4f}, Loss: {:.4f}.".format(f1,
                                                                                                                  pre,
                                                                                                                  recall,
                                                                                                                  epoch_acc,
                                                                                                                  epoch_loss))
        ...
        self.log.info(
            mode + ": F1: {:.4f}, Precision: {:.4f}, Recall : {:.4f}, Accuracy: {:.4f}, Loss: {:.4f}.".format(f1, pre,
                                                                                                              recall,
                                                                                                              epoch_acc,
                                                                                                              epoch_loss))
        self.log.info(
            mode + "-macro: F1: {:.4f}, Precision: {:.4f}, Recall : {:.4f}.".format(f1_macro, pre_macro, recall_macro))
        return {
            "confusion_matrix": conf.tolist(),
            "f1_score": f1.item(),
            "precision_score": pre.item(),
    ...
    def Init(self):
        ...
        self.model = model._models[self.opt["modelopt"]["name"]](self.opt["modelopt"],self.MSG_config,requirements)
